import Image from "next/image";

export default function RegisterPage() {
  return (
    <main className="min-h-screen flex items-center justify-center bg-linear-to-br from-slate-800 via-slate-900 to-black px-4 py-8">
      
      {/* LANDSCAPE CONTAINER */}
      <div className="w-full max-w-3xl h-auto md:h-[520px] bg-slate-700/50 backdrop-blur-sm rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        
        {/* LEFT */}
        <div className="w-full md:w-1/2 bg-[#00ADB5] flex flex-col items-center justify-center p-6 text-center">
          <Image
            src="/OBJECTS.png"
            alt="Logo"
            width={70}
            height={70}
            priority
            className="rounded-xl mb-4 md:w-[90px] md:h-[90px] md:mb-6"
          />

          <h2 className="text-2xl md:text-4xl font-bold text-white mb-2 md:mb-4">
            BrowStuff
          </h2>

          <p className="text-xs md:text-sm text-white font-medium leading-relaxed">
            Daftar sebagai vendor dan <br />
            mulai sewakan barangmu!
          </p>
        </div>

        {/* RIGHT */}
        <div className="w-full md:w-1/2 bg-[#222831] flex justify-center px-4 py-8">
          
          {/* CARD ABU */}
          <div className="w-full max-w-xs min-h-[440px] bg-[#393E46] rounded-xl px-6 py-7 shadow-xl flex flex-col">
            
            {/* TITLE */}
            <h1 className="text-center text-3xl font-bold drop-shadow-[1px_4px_0px_#98B3DF] mb-6">
              <span className="text-cyan-400">REGISTER</span>
            </h1>

            {/* FORM */}
            <div className="space-y-4">
              <div>
                <label className="block text-xs text-white mb-1">
                  Nama Vendor
                </label>
                <input className="w-full border-2 border-white rounded-md px-3 py-2 text-xs bg-transparent text-white focus:outline-none focus:border-cyan-400" />
              </div>

              <div>
                <label className="block text-xs text-white">
                  Email
                </label>
                <input className="w-full border-2 border-white rounded-md px-3 py-2 text-xs bg-transparent text-white focus:outline-none focus:border-cyan-400" />
              </div>

              <div>
                <label className="block text-xs text-white mb-1">
                  Phone Number
                </label>
                <input className="w-full border-2 border-white rounded-md px-3 py-2 text-xs bg-transparent text-white focus:outline-none focus:border-cyan-400" />
              </div>

              <div>
                <label className="block text-xs text-white mb-1">
                  Alamat Vendor
                </label>
                <textarea
                  rows={3}
                  className="w-full border-2 border-white rounded-md px-2 text-xs bg-transparent text-white resize-none focus:outline-none focus:border-cyan-400"
                />
              </div>
            </div>

            {/* ACTION */}
            <button className="mt-6 w-full bg-white hover:bg-gray-100 text-[#00ADB5] font-bold py-2.5 rounded-md text-xs transition">
              Sign Up
            </button>

            {/* LOGIN ALT */}
            <div className="mt-4 text-center text-gray-400 text-xs">
              Or Login With
            </div>

            <div className="mt-3 flex justify-center">
              <button className="w-11 h-11 bg-slate-600/50 rounded-md flex items-center justify-center hover:bg-slate-600 transition">
                {/* google svg */}
                <svg className="w-5 h-5" viewBox="0 0 24 24">
                  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22z"/>
                  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
              </button>
            </div>
          </div>
        </div>
      </div>
    </main>
  );
}
