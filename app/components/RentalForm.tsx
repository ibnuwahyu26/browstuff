...
export default function RentalForm({
  ...
  const [itemsWithDays, setItemsWithDays] = useState(
    items.map((item) => ({ ...item, days: 2 }))
  )

  // Fungsi tambah hari
  const handleIncrease = (id: number) => {
    setItemsWithDays((prev) =>
      prev.map((item) =>
  ...
  const handleDecrease = (id: number) => {
    setItemsWithDays((prev) =>
      prev.map((item) =>
        item.id === id ? { ...item, days: Math.max(1, item.days - 1) } : item
      )
    )
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1e2530] rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        {step === 2 ? (
          <PaymentForm
            selectedItems={itemsWithDays} // pakai itemsWithDays
            onClose={() => setStep(1)}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <button
                            type="button"
                            className="bg-[#00BCD4] text-white text-xs font-bold px-2 py-1 rounded"
                ...
                <div>
                  ...
                  <label className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-[#455A64] border border-[#607D8B] rounded-lg text-[#00BCD4] cursor-pointer hover:bg-[#546E7A] transition">
                    ...
                    <span className="font-semibold">
                      {formData.fotoKTP ? formData.fotoKTP.name : "Upload Foto KTP"}
                    </span>
                    <input
                      type="file"
                      accept="image/*"
